  ...
  <head>
    ...
    <div class="courses">
      ...
      <v-row style="display: flex; justify-content: space-evenly;">
        <v-col class="brd quick vcolcolor" cols="8">
          <!-- <div class="quiz-box"> -->
        ...
        <div v-if="scheduledQuizzes.length > 0">
          ...
          <ul>
            <v-chip-group column>
            <li v-for="quiz in scheduledQuizzes" :key="quiz.id" @click="takeQuiz(quiz.id)" class="lis">
              <v-chip class="ma-2"  text-color="white" :model-value="true" prepend-icon="mdi-checkbox-marked-circle">
                <template v-slot:prepend>
                   <v-avatar color="#AED581"> {{ quiz.id }} </v-avatar>
                </template>
                    <b>{{ quiz.name }}</b>  
              </v-chip>
            </li>
